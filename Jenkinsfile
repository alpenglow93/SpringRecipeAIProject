	...
	stages {
		...
		*/
		...
		stage('Docker Build') {
			steps {
				sh '''
					docker build -t alpenglow93/ai-app:latest . 
					'''
			}
		}
		...
		stage('Docker Push') {
			steps {
				sh '''
					docker push alpenglow93/ai-app:latest
					'''
			}
		}
		...
		stage('Docker Run') {
			steps {
				sh '''
					docker run -d --name ai-app -p 9090:9090 --env-file .env alpenglow93/ai-app:latest
					'''
			}
		}
